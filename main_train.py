...
parser.add_argument('-suffix','--suffix', default='n', type=str,help='suffix')
...
def main():
    ...
    for epoch in range(args.epochs):
        loss, acc, avg_spikes = train(model, device, train_loader, criterion, optimizer, args.time)
        logger.info('Epoch:[{}/{}]\t loss={:.5f}\t acc={:.3f}\t avg_spikes={:.3f}'.format(
            epoch, args.epochs, loss, acc, avg_spikes))
        scheduler.step()
        tmp, test_avg_spikes = val(model, test_loader, device, args.time)
        logger.info('Epoch:[{}/{}]\t Test acc={:.3f}\t Test avg_spikes={:.3f}\n'.format(epoch, args.epochs, tmp, test_avg_spikes))
